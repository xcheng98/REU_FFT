...
int cuFFT32(int N, float* X, float* FX, int B){
    // Allocate unified momory for input and output
    int mem_size = N * N * B *sizeof(float);
    ...
    result = cufftXtMakePlanMany(plan, 2, input_size_long, NULL, 1, 1, \
                         CUDA_R_32F, NULL, 1, 1, CUDA_R_32F, B, \
                         &workSize, CUDA_R_32F);
    if (result != CUFFT_SUCCESS)
    {
...
int cuFFT16(int N, half* X, half* FX, int B){
    // Allocate unified momory for input and output
    int mem_size = N * N * B *sizeof(half);
    half *d_idata, *d_odata;
    checkCudaErrors(cudaMalloc((void **) &d_idata, mem_size));
    checkCudaErrors(cudaMalloc((void **) &d_odata, mem_size));
    ...
    result = cufftXtMakePlanMany(plan, 2, input_size_long, NULL, 1, 1, \
                         CUDA_R_16F, NULL, 1, 1, CUDA_R_16F, B, \
                         &workSize, CUDA_R_16F);
    if (result != CUFFT_SUCCESS)
    {
...
int main(int argc, char **argv)
{
    ...
    float* X_32 = new float[n * n * batch]();
    float* FX_32 = new float[n * n * batch]();
    half* X_16 = new half[n * n * batch]();
    half* FX_16 = new half[n * n * batch]();
