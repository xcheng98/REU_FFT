 ...
 * Using magma-like kernel for transposition
...
/*
 * The transpose kernel in magma library
 * Using tricks to reduce data transferring time
 * Can be tuned by specifying thread numbers per block
*/
template<typename T, const int TY>
__global__ void transpose4_kernel(int n, int stride, T *A, T *AT){
   int batchid = blockIdx.y * stride;
   __shared__ T sA[4][TY+1];
   int tx = threadIdx.x%4, ty = threadIdx.x/4;

   A += batchid + 4*TY*blockIdx.x;
   AT+= batchid +   TY*blockIdx.x;

   if (ty+TY*blockIdx.x < n)
      sA[tx][ty] = A[threadIdx.x];
   __syncthreads();
   tx = threadIdx.x%TY;
   ty = threadIdx.x/TY;
   if (tx +TY*blockIdx.x < n)
     AT[tx + n*ty] = sA[ty][tx];
}

template<typename T, const int TY>
void magmablas_transpose4(int n, int stride, T *A, T *AT, int batchCount){
     dim3 grid((n+TY-1)/TY, batchCount);
     transpose4_kernel<T,TY><<<grid, 4*TY>>>(n, stride, A, AT);
}
...
FFT_S gfft_recursion(int N, float* X_re, float* X_im, float*& FX_re, float*& FX_im, int B) 
{
    ...
    magmablas_transpose4<float, 32>(N/4, N, X_re, buffer, B);
    ...
    magmablas_transpose4<float, 32>(N/4, N, X_im, buffer, B);
