 ...
 * To be called by 2D fft
 * Input should be fp32 vectors with size equals to the power of 4
 * Input and output vector should be in device or unified memory
...
FFT_S gfft_init(int SIZE, int BATCH);
...
FFT_S gfft(int SIZE, float*& X_re, float*& X_im, float*& FX_re, float*& FX_im, int BATCH)
{
    ...
    // Call gfft function
    fft_status = gfft_recursion(SIZE, X_re, X_im, FX_re, FX_im, BATCH);
    if (fft_status != FFT_SUCCESS){
        fprintf(stderr, "!!!!! 1D gFFT execution error.\n");
        exit(1);
    }
