...
FFT_S fft4(fft::VectorF X_re, fft::VectorF X_im, fft::VectorF FX_re, fft::VectorF FX_im) 
{
    ...
    status = cublasSetVector(4, sizeof(X_im.array[0]), X_im.array, 1, dev_input + 4, 1);
    if (status != CUBLAS_STATUS_SUCCESS) {
    ...
    alpha = -1.0f;
    status = cublasSaxpy(handle, 4, &alpha, dev_result2 + 4, 1, dev_result1, 1);
    if (status != CUBLAS_STATUS_SUCCESS) {
        fprintf(stderr, "!!!! CUBLAS kernel execution error (ac - bd).\n");
        return FFT_FAILURE;
    }
    alpha = 1.0f;
    status = cublasSaxpy(handle, 4, &alpha, dev_result2, 1, dev_result1 + 4, 1);
    if (status != CUBLAS_STATUS_SUCCESS) {
    ...
    status = cublasGetVector(4, sizeof(FX_im.array[0]), dev_result1 + 4, 1, FX_im.array, 1);
    if (status != CUBLAS_STATUS_SUCCESS) {
