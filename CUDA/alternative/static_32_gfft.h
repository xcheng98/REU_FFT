...
FFT_S gfft_32_recursion(int N, float* X_re, float* X_im, float*& FX_re, float*& FX_im, int B) 
{
    ...
    myTranspose<<<blockPerGrid1, threadsPerBlock1>>>(4, N / 4, X_re, buffer_32, B);
    ...
    temp = FX_re; FX_re = buffer_32; buffer_32 = temp;

    //// Transpose imaginary matrix
    myTranspose<<<blockPerGrid1, threadsPerBlock1>>>(4, N / 4, X_im, buffer_32, B);
    cerror = cudaGetLastError();
    if (cerror != cudaSuccess)
    {
    ...
    temp = FX_im; FX_im = buffer_32; buffer_32 = temp;
